...
namespace WxPayAPI
{
    public class MicroPay
    {
        ...
        * @return 刷卡支付结果
        */
        public static string Run(string body, string total_fee, string auth_code,string OrderSN)
        {
            ...
            data.SetValue("out_trade_no", OrderSN);//产生随机的商户订单号
            ...
            {
                //支付成功更改状态为1
                NH.BLL.Queryorder.Update(new NH.Model.Queryorder() { OrderNumber = PayDetail.order, Status = "1" });

                //支付成功清空购物车
                NH.BLL.Car.DeleteUId(PayDetail.uid);
