...
namespace NH.DAL
{
	...
	/// Degree
	/// </summary>
	public static partial class Degree
	{
		#region 是否存在
		/// <summary>
		/// 是否存在
		...
		public static bool Exists(NH.Model.Degree model)
		{
			...
			strSql.Append("select count(1) from [Degree](nolock)");
		...
		/// 增加一条数据
		/// </summary>
		public static bool Add(NH.Model.Degree model)
		{
			...
			if(model.DegreeName != null){
				...
				parameter = new SqlParameter("@DegreeName",SqlDbType.NVarChar,10);
            ...
            string strSql = string.Format("insert into [Degree] ({0}) values ({1})", colList.ToString().TrimEnd(','), colParamList.ToString().TrimEnd(','));
		...
		/// 获取SQL及参数
		/// </summary>
		public static string GetSql(NH.Model.Degree model,ref SqlParameter[] parameters,bool isUpdate)
		{
			StringBuilder colList = new StringBuilder();
			...
			if(model.DegreeName != null){
				...
				SqlParameter parameter = new SqlParameter("@DegreeName",SqlDbType.NVarChar,10);
				parameter.Value = model.DegreeName;
				paramList.Add(parameter);
			}
		...
		{
			NH.Model.Degree model = new NH.Model.Degree();
			model.Id = Id;
		...
		public static NH.Model.Degree GetModel(NH.Model.Degree model)
		{
			SqlParameter[] parameters = null;
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select Id, DegreeName, Sort ");			
