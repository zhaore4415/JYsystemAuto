    ...
    public class JsonHelper
    {
        ...
        public static string CreateJsonParameters(DataTable dt, bool displayCount, int totalcount)
        {
            ...
            if (dt != null)
            {
                ...
                return JsonString.ToString().Replace("\n", "").Replace("\r", "").Replace("\t", "");//.Replace("/\"","\\\""); System.Web.HttpUtility.HtmlDecode();
            }
            else
            {
                return null;
        ...
        /// <summary>
        /// 将DataTable中的数据转换成JSON格式,显示汇总，汇总字段
        /// </summary>
        /// <param name="dt">数据源DataTable</param> 
        /// <param name="displayCount">是否输出数据总条数</param>
        /// <param name="totalcount">JSON中显示的数据总条数</param>
        /// <returns></returns>
        public static string CreateJsonParameters_ShowFooter(DataTable dt, bool displayCount, int totalcount, ArrayList list_footer, bool diaplayFooter)
        {
            StringBuilder JsonString = new StringBuilder();
            //Exception Handling        
            ...
            if (dt != null)
            {
                ...
                if (diaplayFooter)
                {
                    if (list_footer.Count > 0)
                    {
                        JsonString.Append(",");
                        JsonString.Append("\"footer\":");
                        JsonString.Append("[{");
                        int cols_ct = 0;
                        foreach (string col in list_footer)
                        {
                            cols_ct++;
                            if (cols_ct == list_footer.Count)
                            {
                                JsonString.Append("\"" + col.ToString() + "\":\"\"");
                            }
                            else
                            {
                                JsonString.Append("\"" + col.ToString() + "\":\"\",");
        ...
        public static string CreateJsonParameters_treegrid(DataTable dt, bool displayCount, int totalcount)
        {
            StringBuilder JsonString = new StringBuilder();
            //Exception Handling        
            ...
            if (dt != null)
            {
                ...
                JsonString.Append("}");
                return JsonString.ToString().Replace("\n", "").Replace("\r", "").Replace("\t", "").Replace("PARENTID", "_parentId").Replace("PARENTCURRENTID", "id");
            }
            else
            {
                return null;
        ...
        /// <summary>
        /// 将DataTable中的数据转换成JSON格式  selected控制json
        /// </summary>
        /// <param name="dt">数据源DataTable</param>
        /// <returns></returns>

        public static string CreateJson_sel(DataTable dt)
        {
            StringBuilder JsonString = new StringBuilder();
            //Exception Handling       
            if (dt != null)
            {
                ...
                JsonString.Append("]");
                return JsonString.ToString().Replace("\n", "").Replace("\r", "").Replace("\t", "");  //System.Web.HttpUtility.HtmlDecode()
            }
            else
            {
                return null;
        ...
        /// <param name="jsonData">json数据</param>  
        /// <returns></returns>  
        public static string DictionaryToJson<T>(Dictionary<string, DateTime> dic)
        {
            //实例化JavaScriptSerializer类的新实例  
            JavaScriptSerializer jss = new JavaScriptSerializer();
            try
