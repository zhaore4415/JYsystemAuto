...
namespace Maticsoft.Common
{
    ...
    public class DataToExcel
    {
        ...
        {
            string FilePath = System.Web.HttpContext.Current.Server.MapPath("/Upload/") + "ReportFile\\";
            //利用excel对象
            ...
            try
            {
                ...
                {
                    filename = dte.DataExcel(MyData.Tables[0],"dengji", ReportTitle, FilePath, nameList);
                }
            }
            catch
            {}
            if (filename != "")
            {
                System.Web.HttpContext.Current.Response.Redirect("/Upload/ReportFile/" + filename, true);
            }
        }
        ...
        /// <returns></returns>
        public string DataExcel(System.Data.DataTable dt,string tianname, string strTitle, string FilePath, Hashtable nameList)
        {
            COM.Excel.cExcelFile excel = new COM.Excel.cExcelFile();
            //ClearFile(FilePath);
            string filename = tianname + DateTime.Now.ToString("yyyyMMddHHmmssff") + ".xls";
